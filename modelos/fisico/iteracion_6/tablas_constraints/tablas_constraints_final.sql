CREATE TABLE terminos_condiciones (
    id_terminos_condiciones INT PRIMARY KEY IDENTITY(1,1),
    ...
    CONSTRAINT chk_terminos_titulo CHECK (LEN(titulo) > 0),
    CONSTRAINT chk_terminos_contenido CHECK (LEN(contenido_texto) > 0),
    CONSTRAINT chk_terminos_version CHECK (version > 0),
    CONSTRAINT chk_terminos_vigencia CHECK (fecha_fin_vigencia IS NULL OR fecha_fin_vigencia >= fecha_inicio_vigencia)
);
    CREATE UNIQUE INDEX IX_terminos_version_actual
...
CREATE TABLE idioma (
    ...
    codigo_iso VARCHAR(2) NULL UNIQUE,
    codigo_iso3 VARCHAR(3) NULL,
    
    CONSTRAINT chk_idioma_nombre CHECK (LEN(nombre) > 0),
    CONSTRAINT chk_codigo_iso CHECK (codigo_iso IS NULL OR LEN(codigo_iso) = 2),
    CONSTRAINT chk_codigo_iso3 CHECK (codigo_iso3 IS NULL OR LEN(codigo_iso3) = 3)
);
...
CREATE TABLE persona (
    ...
    unidad_distancia VARCHAR(20) NOT NULL DEFAULT 'km',
    notificacion_push BIT NOT NULL DEFAULT 1,
    notificacion_email BIT NOT NULL DEFAULT 1,
    id_terminos_condiciones INT NULL,
    id_idioma INT NULL,
    ...
    -- Usuario: No vacío, mínimo 4 caracteres, sin espacios
    CONSTRAINT CHK_persona_usuario_valido
    CHECK (LEN(TRIM(usuario)) >= 4 AND usuario NOT LIKE '%[ ]%' AND usuario LIKE '%[A-Za-z0-9_.-]%'),
    
    -- Contraseña: Mínimo 8 caracteres
    CONSTRAINT CHK_persona_contraseña_minima
    CHECK (LEN(contraseña) >= 8),
    ...
    CONSTRAINT chk_persona_unidad_distancia CHECK (unidad_distancia IN ('km', 'millas', 'metros')),
    ...
    CONSTRAINT FK_persona_idioma
    FOREIGN KEY (id_idioma) REFERENCES idioma(id_idioma)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,

    CONSTRAINT FK_persona_terminos FOREIGN KEY (id_terminos_condiciones)
        REFERENCES terminos_condiciones(id_terminos_condiciones)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION
);
...
CREATE TABLE punto_alquiler (
    ...
    CONSTRAINT fk_punto_admin FOREIGN KEY (id_admin)
        REFERENCES administrador(id_admin)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
...
CREATE TABLE contacto_punto (
   ...
   UNIQUE (id_contacto, id_punto_alquiler)
);
...
CREATE TABLE capacidad (
    ...
    CONSTRAINT fk_capacidad_admin FOREIGN KEY (id_admin)
        REFERENCES administrador(id_admin)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION
    )
...
CREATE TABLE cobertura_seguro (
    ...
    CONSTRAINT chk_fecha_coherencia_cobertura
CHECK (
    fecha_fin_vigencia IS NULL
    OR fecha_fin_vigencia >= fecha_inicio_vigencia
    OR estado = 'vencido'  -- Permitimos fin < inicio si está vencido
),
...
CREATE TABLE fotografia (
    ...
    tipo_fotografia VARCHAR(20) NOT NULL,
    ...
    CONSTRAINT CHK_fotografia_tipo CHECK (tipo_fotografia IN ('ruta', 'bicicleta', 'punto_alquiler', 'resenia'))
)
...
CREATE TABLE fotografia_bicicleta (
    id_fotografia_bicicleta INT PRIMARY KEY,
    id_bicicleta INT NOT NULL,
    tipo_vista VARCHAR(50) NULL,
    
    ValidFrom DATETIME2 GENERATED ALWAYS AS ROW START HIDDEN,
    ValidTo DATETIME2 GENERATED ALWAYS AS ROW END HIDDEN,
    PERIOD FOR SYSTEM_TIME (ValidFrom, ValidTo),
    
    CONSTRAINT fk_fotografia_bicicleta_fotografia FOREIGN KEY (id_fotografia_bicicleta) 
    ...
    CONSTRAINT uk_fotografia_bicicleta UNIQUE (id_fotografia_bicicleta, id_bicicleta),
    
  CONSTRAINT ck_fotografia_bicicleta_tipo_vista
CHECK (
    tipo_vista IS NULL
    OR tipo_vista IN ('frontal', 'lateral', 'trasera', 'detalle', 'otro')
)
)
...
CREATE TABLE fotografia_punto (
    id_fotografia_punto INT PRIMARY KEY,
    id_punto_alquiler INT NOT NULL,
    tipo_vista VARCHAR(50) NULL,
    ...
    CONSTRAINT fk_fotografia_punto_fotografia FOREIGN KEY (id_fotografia_punto) 
        REFERENCES fotografia(id_fotografia) 
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    ...
    CONSTRAINT uk_fotografia_punto UNIQUE (id_fotografia_punto, id_punto_alquiler),
    CONSTRAINT ck_fotografia_punto_tipo_vista
CHECK (
    tipo_vista IS NULL
    OR tipo_vista IN ('entrada', 'salida', 'estacionamiento', 'general', 'otro')
)
)
...
CREATE TABLE ruta_turistica (
    ...
    descripcion TEXT NULL,
    url_inicio_googlemaps VARCHAR(500) NULL,
    url_fin_googlemaps VARCHAR(500) NULL,
    referencia_inicio VARCHAR(255) NULL,
    ...
    estado VARCHAR(20) NOT NULL DEFAULT 'activo',
    fecha_creacion DATETIME NOT NULL DEFAULT GETDATE(),
    calificacion_promedio INT NULL,
    ...
    CONSTRAINT chk_ruta_nombre CHECK (LEN(nombre) > 0),
    CONSTRAINT chk_ruta_distancia CHECK (distancia_total > 0),
    CONSTRAINT chk_ruta_duracion CHECK (duracion_estimada > '00:00:00'),
    CONSTRAINT chk_ruta_nivel_dificultad CHECK (nivel_dificultad IN ('muy_facil', 'facil', 'moderada', 'dificil', 'muy_dificil')),
    CONSTRAINT chk_ruta_estado CHECK (estado IN ('activo', 'inactivo', 'mantenimiento')),
    CONSTRAINT CHK_ruta_calificacion CHECK (calificacion_promedio IS NULL OR (calificacion_promedio >= 1.00 AND calificacion_promedio <= 5.00)),
    CONSTRAINT CHK_ruta_total_resenas CHECK (total_resenas >= 0),
    CONSTRAINT CHK_ruta_url_inicio CHECK (url_inicio_googlemaps IS NULL OR url_inicio_googlemaps LIKE 'http%'),
    CONSTRAINT CHK_ruta_url_fin CHECK (url_fin_googlemaps IS NULL OR url_fin_googlemaps LIKE 'http%')
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.ruta_turistica_historial));
...
CREATE TABLE guia_turistico (
    ...
    -- id_guia es PK Y FK a la vez (Herencia TPT)
    CONSTRAINT FK_guia_persona FOREIGN KEY (id_guia) 
        REFERENCES persona(id_persona)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT chk_guia_numero_licencia CHECK (LEN(numero_licencia) > 0),
    CONSTRAINT chk_guia_tarifa_base CHECK (tarifa_base > 0),
    CONSTRAINT chk_guia_estado CHECK (estado IN ('activo', 'inactivo', 'suspendido', 'licencia_vencida')),
    CONSTRAINT chk_guia_fecha_no_futura CHECK (fecha_asignacion <= GETDATE())
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.guia_turistico_historial));
...
CREATE TABLE reserva (
    ...
    CONSTRAINT FK_reserva_ruta FOREIGN KEY (id_ruta)
        REFERENCES ruta_turistica(id_ruta)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,

    CONSTRAINT FK_reserva_guia FOREIGN KEY (id_guia)
    ...
    CONSTRAINT CHK_reserva_guia_tarifa_obligatoria CHECK ((id_guia IS NULL AND tarifa_guia IS NULL) OR (id_guia IS NOT NULL AND tarifa_guia IS NOT NULL)),
    CONSTRAINT CHK_reserva_fecha_no_futura CHECK (fecha_reserva <= GETDATE()),
    CONSTRAINT chk_reserva_fechas CHECK (fecha_fin IS NULL OR fecha_inicio IS NULL OR fecha_fin >= fecha_inicio)
)
...
CREATE TABLE guia_idioma (
    id_guia_idioma INT PRIMARY KEY IDENTITY(1,1),
    ...
    CONSTRAINT FK_guia_idioma_guia FOREIGN KEY (id_guia) 
        REFERENCES guia_turistico(id_guia)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT FK_guia_idioma_idioma FOREIGN KEY (id_idioma) 
        REFERENCES idioma(id_idioma)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
    
    CONSTRAINT UQ_guia_idioma_combinacion UNIQUE (id_guia, id_idioma),
    CONSTRAINT chk_guia_idioma_estado CHECK (estado IN ('activo', 'inactivo')),
    CONSTRAINT chk_guia_idioma_fecha_no_futura CHECK (fecha_asignacion <= GETDATE())
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.guia_idioma_historial));



CREATE TABLE guia_ruta (
    id_guia_ruta INT PRIMARY KEY IDENTITY(1,1),
    id_guia INT NOT NULL,
    id_ruta INT NOT NULL,
    veces_realizada INT NOT NULL DEFAULT 0,
    estado VARCHAR(20) NOT NULL DEFAULT 'activo',
    fecha_asignacion DATETIME NOT NULL DEFAULT GETDATE(),
    ...
    CONSTRAINT FK_guia_ruta_guia FOREIGN KEY (id_guia) 
        REFERENCES guia_turistico(id_guia)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT FK_guia_ruta_ruta FOREIGN KEY (id_ruta) 
        REFERENCES ruta_turistica(id_ruta)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
    
    CONSTRAINT UQ_guia_ruta_combinacion UNIQUE (id_guia, id_ruta),
    CONSTRAINT chk_guia_ruta_veces_realizada CHECK (veces_realizada >= 0),
    CONSTRAINT chk_guia_ruta_estado CHECK (estado IN ('activo', 'inactivo', 'suspendido')),
    CONSTRAINT chk_guia_ruta_fecha_no_futura CHECK (fecha_asignacion <= GETDATE())
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.guia_ruta_historial));

CREATE TABLE resenia_ruta (
    id_resenia_ruta INT PRIMARY KEY IDENTITY(1,1),
    id_reserva INT NOT NULL,
    id_ruta INT NOT NULL,
    id_cliente INT NOT NULL,
    ...
    CONSTRAINT FK_resenia_reserva FOREIGN KEY (id_reserva) 
        REFERENCES reserva(id_reserva)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT FK_resenia_ruta FOREIGN KEY (id_ruta) 
        REFERENCES ruta_turistica(id_ruta)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,

    CONSTRAINT fk_resenia_ruta_cliente FOREIGN KEY(id_cliente)
        REFERENCES cliente(id_cliente)
        ON DELETE NO ACTION
        ON UPDATE NO ACTION,
    
    CONSTRAINT chk_resenia_calificacion CHECK (calificacion IS NULL OR (calificacion >= 1 AND calificacion <= 5)),
    CONSTRAINT chk_resenia_estado CHECK (estado IN ('activo', 'eliminado'))
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.resenia_ruta_historial));


CREATE TABLE foto_resenia (
    id_foto_resenia INT PRIMARY KEY,
    id_resenia_ruta INT NOT NULL,
    orden INT NOT NULL DEFAULT 0,
    
    CONSTRAINT FK_foto_resenia_fotografia FOREIGN KEY (id_foto_resenia) 
        REFERENCES fotografia(id_fotografia)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT FK_foto_resenia_resenia FOREIGN KEY (id_resenia_ruta) 
        REFERENCES resenia_ruta(id_resenia_ruta)
        ON DELETE CASCADE
        ON UPDATE NO ACTION,
    
    CONSTRAINT UQ_foto_resenia_fotografia UNIQUE (id_foto_resenia),
    CONSTRAINT chk_foto_resenia_orden CHECK (orden >= 0)
);
...
CREATE TABLE foto_ruta (
    ...
    punto_interes VARCHAR(200) NULL,
    ...
    CONSTRAINT FK_foto_ruta_fotografia
    FOREIGN KEY (id_fotografia) REFERENCES fotografia(id_fotografia)
    ON DELETE CASCADE
    ON UPDATE CASCADE,

    CONSTRAINT FK_foto_ruta_ruta
    FOREIGN KEY (id_ruta) REFERENCES ruta_turistica(id_ruta)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
    
    CONSTRAINT CHK_foto_ruta_tipo
    CHECK (tipo_foto IN ('principal', 'paisaje', 'punto_interes', 'sendero', 'dificultad', 'fauna', 'flora')),
    
    CONSTRAINT CHK_foto_ruta_temporada
    CHECK (temporada IS NULL OR temporada IN ('primavera', 'verano', 'otoño', 'invierno')),
    
    CONSTRAINT CHK_foto_ruta_punto_interes
    CHECK (punto_interes IS NULL OR LEN(TRIM(punto_interes)) > 0),
    
)
WITH (SYSTEM_VERSIONING = ON (HISTORY_TABLE = dbo.foto_ruta_historial));